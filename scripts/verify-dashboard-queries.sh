...
queries=(
    "cerulea_consensus_current_epoch"
    "cerulea_consensus_active_validators" 
    "rate(cbc_author_mismatch_total[5m])"
    "rate(cbc_epoch_transitions_total[1h])"
    "histogram_quantile(0.95, rate(cerulea_block_production_time_seconds_bucket[5m]))"
    "substrate_rpc_sessions_opened"
    "substrate_rpc_sessions_closed"
    "cerulea_consensus_total_reserved_stake"
    "cerulea_consensus_total_rewards_distributed"
    "cerulea_consensus_total_slashed_amount"
