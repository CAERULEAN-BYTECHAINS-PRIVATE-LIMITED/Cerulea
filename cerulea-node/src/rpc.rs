...
use cerulea_runtime::{opaque::Block, AccountId, Balance, Nonce};
...
pub struct FullDeps<C, P, B> {
    ...
    pub consensus_metrics: Option<cerulea_consensus::metrics::ConsensusMetrics>,
    pub backend: Arc<B>,
}
...
pub struct PosRpcApiImpl<C> {
    client: Arc<C>,
    consensus_metrics: Option<cerulea_consensus::metrics::ConsensusMetrics>,
}
...
impl<C> PosRpcApiImpl<C> {
    ...
    pub fn new_with_metrics(client: Arc<C>, consensus_metrics: cerulea_consensus::metrics::ConsensusMetrics) -> Self {
        Self { 
            client,
    ...
    fn describe(&self) -> RpcResult<Vec<RpcMethodDescription>> {
        ...
        let methods = vec![
            ...
            RpcMethodDescription {
                name: "cerulea_getCurrentEpoch".to_string(),
                description: "Get the current epoch number".to_string(),
                params: vec![],
                returns: "u32".to_string(),
            },
            RpcMethodDescription {
                name: "cerulea_getValidatorProfile".to_string(),
