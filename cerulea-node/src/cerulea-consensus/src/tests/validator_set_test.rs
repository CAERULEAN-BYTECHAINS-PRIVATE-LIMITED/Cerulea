...
mod tests {
    ...
    fn test_validator_set_rotation() {
        setup_consensus_test(MockConsensusConfig::default()).execute_with(|| {
            let initial_validators = pallet_cerulea_dcf::ValidatorSet::<Test>::get();
            ...
            if updated_validators.try_push(new_validator).is_ok() {
                pallet_cerulea_dcf::ValidatorSet::<Test>::put(&updated_validators);
                
                // Create state for new validator
                let new_state = pallet_cerulea_dcf::ValidatorState {
                    last_active_epoch: 0,
                ...
                pallet_cerulea_dcf::ValidatorStates::<Test>::insert(&new_validator, new_state);
                pallet_cerulea_dcf::ValidatorStake::<Test>::insert(&new_validator, 1000u128);
    ...
    fn test_validator_removal() {
        setup_consensus_test(MockConsensusConfig::default()).execute_with(|| {
            ...
            if let Some(pos) = updated_validators.iter().position(|v| *v == validator_to_remove) {
                ...
                let mut active_validators = pallet_cerulea_dcf::ActiveValidators::<Test>::get();
                if let Some(pos) = active_validators.iter().position(|v| *v == validator_to_remove) {
                    active_validators.remove(pos);
                    pallet_cerulea_dcf::ActiveValidators::<Test>::put(&active_validators);
                }
                ...
                let current_active = pallet_cerulea_dcf::ActiveValidators::<Test>::get();
    ...
    fn test_validator_epoch_history() {
        setup_consensus_test(MockConsensusConfig::default()).execute_with(|| {
            ...
            let epoch_stats = pallet_cerulea_dcf::EpochStats {
                epoch: 1,
                stake_score: 1100,
                inference_score: 850,
                final_score: 950,
