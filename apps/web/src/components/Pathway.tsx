...
export interface PathwayDefinition {
  id: PathwayId;
  /** Four or five words, for the chip face. */
  title: string;
...
export const PATHWAYS: Record<PathwayId, PathwayDefinition> = {
  ...
  P5: {
    id: 'P5',
    title: 'Sub ₹200 crore restriction',
...
const KNOWN = new Set<string>(PATHWAY_IDS);

/** Only ever render a pathway the chain actually recorded. */
export function asPathwayId(value: string | null | undefined): PathwayId | undefined {
  if (!value) return undefined;
  const upper = value.toUpperCase();
  return KNOWN.has(upper) ? (upper as PathwayId) : undefined;
}

/**
 * Which of P1–P12 a decision took.
 *
 * Accent-toned, never a status colour: the pathway is HOW the decision was reached, not
 * whether it passed. Hovering or focusing gives the PoC document's own sentence for it.
 */
    ...
    >
      ...
      >
        <Chip tone="accent">
          ...
          {withTitle && <span className="font-normal">{definition.title}</span>}
        </Chip>
      </button>
    </Tooltip>
  );
